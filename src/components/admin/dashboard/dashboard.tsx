...
import { Activity, Car, CircleDollarSign, Users, Loader2, MapPin } from "lucide-react";
...
export default function AdminDashboard({
  ...
  // Listener en tiempo real para conductores disponibles
  useEffect(() => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Mapa de conductores disponibles en tiempo real */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <MapPin className="h-5 w-5" />
            Conductores Disponibles en Tiempo Real
          </CardTitle>
          <CardDescription>
            {availableDrivers.length} conductor{availableDrivers.length !== 1 ? 'es' : ''} disponible{availableDrivers.length !== 1 ? 's' : ''} actualmente
          </CardDescription>
        </CardHeader>
        <CardContent>
